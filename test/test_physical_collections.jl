using LightLattices: cell, lattice, element
...
@testset "C13-enriched diamond unit cell" begin
    ...
    @test phys_dcell.elements == (isotopes[:C13],)
    @test element(phys_dcell, 1) == isotopes[:C13]
    @test_throws ErrorException element(phys_dcell, 2)
    ...
    @test phys_dcell[2] == SVector{3,Float64}(0.25,0.25,0.25)
    @test relative_coordinate(phys_dcell, 2, 1) == SVector{3,Float64}(0.25,0.25,0.25)
...
@testset "F19 and P31 fluorapatite unit cell" begin
    ...
    @test phys_fcell.elements == (isotopes[:F19], isotopes[:P31])
    @test element(phys_fcell, 2) == isotopes[:P31]
    @test_throws ErrorException element(phys_fcell, 3)
    ...
    @test phys_fcell[2] == fcell_vectors[1][2]
    @test phys_fcell[2, 1] == fcell_vectors[1][2]
    @test phys_fcell[3] == fcell_vectors[2][1]
    @test phys_fcell[1,2] == fcell_vectors[2][1]
    @test relative_coordinate(phys_fcell, 5, 2) == fcell_vectors[2][3] - fcell_vectors[1][2]
    @test relative_coordinate(phys_fcell, (3,2), 2) == fcell_vectors[2][3] - fcell_vectors[1][2]
    @test relative_coordinate(phys_fcell, 5, (2,1)) == fcell_vectors[2][3] - fcell_vectors[1][2]
    @test relative_coordinate(phys_fcell, (3,2), (2,1)) == fcell_vectors[2][3] - fcell_vectors[1][2]
...
@testset "F19 cubic lattice" begin
    ...
    @test phys_cubic_lattice_f.elements == (isotopes[:F19],)
    @test phys_cubic_lattice_p.elements == (isotopes[:F19],)
    @test element(phys_cubic_lattice_p, 1) == isotopes[:F19]
    @test_throws ErrorException element(phys_cubic_lattice_p, 2)
    ...
    @test phys_cubic_lattice_p[13,24,182] ≈ 2.725u"Å"*SVector(2, 2, 6)
    @test phys_cubic_lattice_p[CartesianIndex(13,24,182)] ≈ 2.725u"Å"*SVector(2,2,6)
    @test phys_cubic_lattice_p[CartesianIndex(13,24,182), 1] ≈ 2.725u"Å"*SVector(2,2,6)
    @test relative_coordinate(phys_cubic_lattice_f, CartesianIndex(2,2,6), CartesianIndex(1,1,1)) ≈ 2.725u"Å"*SVector(1,1,5)
    @test relative_coordinate(phys_cubic_lattice_f, (CartesianIndex(2,2,6),1), (CartesianIndex(1,1,1),1)) ≈ 2.725u"Å"*SVector(1,1,5)
    @test relative_coordinate(phys_cubic_lattice_f, (2,2,6,1), (1,1,1,1)) ≈ 2.725u"Å"*SVector(1,1,5)
    @test relative_coordinate(phys_cubic_lattice_p, CartesianIndex(13,24,182), CartesianIndex(1,1,1)) ≈ 2.725u"Å"*SVector(1,1,5)
    @test relative_coordinate(phys_cubic_lattice_p, (CartesianIndex(13,24,182),1), (CartesianIndex(1,1,1),1)) ≈ 2.725u"Å"*SVector(1,1,5)
    @test relative_coordinate(phys_cubic_lattice_p, (13,24,182,1), (1,1,1,1)) ≈ 2.725u"Å"*SVector(1,1,5)
...
@testset "C13-enriched diamond lattice" begin
    ...
    @test phys_diamond_lattice_f.elements == (isotopes[:C13],)
    @test phys_diamond_lattice_p.elements == (isotopes[:C13],)
    @test element(phys_diamond_lattice_p, 1) == isotopes[:C13]
    @test_throws ErrorException element(phys_diamond_lattice_p, 2)
    ...
    @test phys_diamond_lattice_p[CartesianIndex(13,24,182), 1] == dpvecs*SVector(2,2,6)
    @test phys_diamond_lattice_p[CartesianIndex(13,24,182), 2] == dpvecs*SVector(2,2,6) + SVector(0.25,0.25,0.25)
    @test phys_diamond_lattice_p[13,24,182, 1] == dpvecs*SVector(2,2,6)
    @test phys_diamond_lattice_p[13,24,182, 2] == dpvecs*SVector(2,2,6) + SVector(0.25,0.25,0.25)
    @test relative_coordinate(phys_diamond_lattice_p, I1, I2) == dpvecs*SVector(1,1,5) - SVector(0.25,0.25,0.25)
    @test relative_coordinate(phys_diamond_lattice_p, I1, I2) == -relative_coordinate(phys_diamond_lattice_p, I2, I1)
    @test relative_coordinate(phys_diamond_lattice_p, I1t, I2t) == dpvecs*SVector(1,1,5) - SVector(0.25,0.25,0.25)
    @test relative_coordinate(phys_diamond_lattice_p, I1t, I2t) == -relative_coordinate(phys_diamond_lattice_p, I2, I1)
...
@testset "F19 and P31 fluorapatite lattice" begin
    ...
    @test phys_fluorapatite_lattice_p.elements == (isotopes[:F19], isotopes[:P31])
    @test element(phys_fluorapatite_lattice_p, 2) == isotopes[:P31]
    @test_throws ErrorException element(phys_fluorapatite_lattice_p, 3)
    ...
    @test phys_fluorapatite_lattice_p[CartesianIndex(13,24,182), 5,2] == fpvecs*SVector(2,2,6) + fcell[5,2]
    @test phys_fluorapatite_lattice_p[13,24,182, 5,2] == fpvecs*SVector(2,2,6) + fcell[5,2]
    @test relative_coordinate(phys_fluorapatite_lattice_p, I1_f, I2_f) ≈ fpvecs*SVector(1,1,5) + fcell[5,2] - fcell[2,1]
    @test relative_coordinate(phys_fluorapatite_lattice_p, I1_f, I2_f) ≈ - relative_coordinate(phys_fluorapatite_lattice_p, I2_f, I1_f)
    @test relative_coordinate(phys_fluorapatite_lattice_p, I1t_f, I2t_f) ≈ fpvecs*SVector(1,1,5) + fcell[5,2] - fcell[2,1]
    @test relative_coordinate(phys_fluorapatite_lattice_p, I1t_f, I2t_f) ≈ - relative_coordinate(phys_fluorapatite_lattice_p, I2t_f, I1t_f)
