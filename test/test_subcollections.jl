...
const diagonal = view(phys_cubic_lattice_f, 1 => reverse(diagonal_indices))
@testset "F19 cubic lattice body diagonal subcollection" begin
    ...
    @test_throws BoundsError view(phys_cubic_lattice_f, 1 => [(CartesianIndex(12,12,12), 1)])
end

const cross = view(phys_diamond_lattice_p, 1 => reverse(diamond_cross_indices))
@testset "C13 diamond seven-cell periodic subcollection" begin
    ...
    @test_throws BoundsError view(phys_diamond_lattice_p, 1 => [(CartesianIndex(0,0,0), 3)])
end

const column = view(phys_fluorapatite_lattice_p,
...
@testset "Fluorapatite column with one F19 and three P31 sites per cell" begin
    ...
    @test_throws BoundsError view(phys_fluorapatite_lattice_p,
