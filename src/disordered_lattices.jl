...
"""
mutable struct DisorderedLattice{D, T, PB, RLT<:RegularLattice{D, T, PB}, AT, TT<:Tuple{Int, Vararg{Int}}, RT} <: AbstractLattice{D, T, PB}
    """
    ...
    function DisorderedLattice(rlattice::RegularLattice{D,T,PB}, ρ::RT, rng=Base.GLOBAL_RNG) where {D,T,PB,RT}
        ...
        new{D,T,PB, typeof(rlattice), typeof(indices), typeof(group_sizes), RT}(rlattice, indices, group_sizes, ρ)
    end
end
...
_initialize_indices(htrait::IsHomogeneous{false}, rlattice::RegularLattice, ρ::Float64, rng) = Tuple((@inbounds group_iterator(htrait, rlattice, ig))|>collect|>x->randsubseq(x,ρ) for ig=1:num_of_groups(rlattice))
_initialize_indices(htrait::IsHomogeneous{false}, rlattice::RegularLattice{D,T,PB,<:InhomogeneousCell{D,T,N}}, ρ::NTuple{N,Float64}, rng) where {D,T,PB,N} = Tuple((@inbounds group_iterator(htrait, rlattice, ig))|>collect|>x->randsubseq(x,ρ[ig]) for ig=1:num_of_groups(rlattice))


## Groups and homogeneity trait.

@inline num_of_groups(dlattice::DisorderedLattice) = num_of_groups(dlattice.rlattice)
@propagate_inbounds function group_size(dlattice::DisorderedLattice, ig::Int)
    @boundscheck check_groupbounds(dlattice, ig)
    dlattice.group_sizes[ig]
end
...
@inline _translate_d_index(::IsHomogeneous{true}, dlattice::DisorderedLattice, i::Int) = dlattice.indices[1][i]
@inline _translate_d_index(::IsHomogeneous, dlattice::DisorderedLattice, i::Int, ig::Int) = (dlattice.indices[ig][i]..., ig)
...
@inline Base.checkindex(::IsHomogeneous{true}, dlattice::DisorderedLattice, i::Int) = 1≤i≤length(dlattice.indices[1])
@inline Base.checkindex(htrait::IsHomogeneous, dlattice::DisorderedLattice, i::Int, ig::Int) = check_group_index(htrait, dlattice, ig) && (1≤i≤length(dlattice.indices[ig]))

@propagate_inbounds function getindex(dlattice::DisorderedLattice, I...)
    @boundscheck checkbounds(dlattice, I...)
    @inbounds dlattice.lattice[_translate_d_index(dlattice, I...)...]
end

@propagate_inbounds relative_coordinate(dlattice::DisorderedLattice, I1::T, I2::T) where {T} = relative_coordinate(dlattice.rlattice, _translate_d_index(I1), _translate_d_index(I2))
...
@propagate_inbounds function group_iterator(::IsHomogeneous, lattice::DisorderedLattice, ig::Int)
    @boundscheck check_group_index(lattice, ig)
    return lattice.indices[ig]
end
